        ...
        <tbody>
          @forelse($leads as $l)
            <tr>
              ...
              <td class="hide-mobile">
                <span
                  class="badge bg-{{ $l->registration_status === 'pending' ? 'warning' : ($l->registration_status === 'converted' ? 'success' : 'secondary') }}">
                  {{ $l->registration_ar }}
                </span>
              </td>
              ...
              <td class="text-end">
                ...
                @if(auth()->user()?->hasPermission('delete_leads'))
                  <form method="POST" action="{{ route('leads.destroy', $l) }}" class="d-inline"
