...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 gap-2">
  ...
  <a href="{{ route('students.index') }}" class="btn btn-outline-secondary rounded-pill px-4 fw-bold">
...
<div class="row g-2 mb-3">
  ...
  <div class="col-6 col-md-3">
    <div class="card border-0 shadow-sm text-center py-2" style="border-right:4px solid #f59e0b !important;border-radius:12px;">
      <div style="font-size:1.6rem;font-weight:900;color:#f59e0b;">{{ $activeCount }}</div>
  ...
  <div class="col-6 col-md-3">
    <div class="card border-0 shadow-sm text-center py-2" style="border-right:4px solid #8b5cf6 !important;border-radius:12px;">
      <div style="font-size:1.6rem;font-weight:900;color:#8b5cf6;">{{ $todayCount }}</div>
...
{{-- ══ الفلاتر ══ --}}
<form class="card card-body border-0 shadow-sm mb-3" method="GET" action="{{ route('students.reports.index') }}" id="reportFilterForm">
  <div class="row g-2 align-items-end">

    <div class="col-12 col-md-3">
      <label class="form-label fw-bold small text-muted">بحث</label>
      <input name="search" value="{{ request('search') }}" class="form-control" placeholder="الاسم / الرقم / الهاتف">
    </div>
    ...
    <div class="col-6 col-md-2">
      ...
      <select name="diploma_id" class="form-select" id="diplomaFilterSelect">
        ...
        @foreach($diplomas as $d)
          <option value="{{ $d->id }}" data-code="{{ $d->code }}"
            @selected(request('diploma_id') == $d->id)>{{ $d->name }} — {{ $d->code }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div class="col-6 col-md-3 d-grid">
      <a href="{{ route('students.reports.index') }}" class="btn btn-outline-secondary fw-bold">
...
<div class="card-body py-2 d-flex justify-content-start align-items-center flex-wrap gap-2">
    <div class="dropdown">
      <button class="btn btn-outline-secondary btn-sm dropdown-toggle fw-bold" type="button"
        ...
        <span class="badge bg-primary rounded-pill ms-1" id="colCountBadge">{{ count($defaultColumns) }}</span>
      </button>
      ...
      <div class="dropdown-menu column-picker-dropdown" aria-labelledby="columnsDropdownBtn">
        ...
        <div class="cp-body">
          ...
          @foreach($availableColumns as $key => $label)
            <label class="cp-item" for="col-{{ $key }}">
              <span class="cp-item-label">
                <i class="bi {{ $columnIcons[$key] ?? 'bi-dot' }}"></i>
                {{ $label }}
              </span>
              <span class="cp-switch">
                <input class="column-toggle" type="checkbox" value="{{ $key }}" id="col-{{ $key }}"
                  @checked(in_array($key, $defaultColumns))>
  ...
  <div class="table-responsive">
    <table class="table table-hover mb-0 align-middle">
      ...
      <tbody>
        ...
        <tr>
          @foreach($availableColumns as $key => $label)
            <td data-col="{{ $key }}">
              @switch($key)
                @case('id') {{ $s->id }} @break
                @case('university_id') <code>{{ $s->university_id }}</code> @break
                @case('full_name') <span class="fw-semibold">{{ $s->full_name }}</span> @break
                @case('latin_name') {{ $s->profile->arabic_full_name ?? '-' }} @break
                @case('phone') {{ $s->phone ?? '-' }} @break
                @case('whatsapp') {{ $s->whatsapp ?? '-' }} @break
                @case('branch') {{ $s->branch->name ?? '-' }} @break
                @case('diplomas') {{ $s->diplomas->pluck('name')->implode('، ') ?: '-' }} @break
                @case('mode') {{ $s->mode_ar }} @break
                @case('status') <span class="badge bg-secondary">{{ $s->status_ar }}</span> @break
                @case('registration_status') <span class="badge bg-info">{{ $s->registration_ar }}</span> @break
                @case('nationality') {{ $s->profile->nationality ?? '-' }} @break
                @case('birth_date') {{ $s->profile->birth_date ?? '-' }} @break
                @case('national_id') {{ $s->profile->national_id ?? '-' }} @break
                @case('crm_source') {{ $s->crm_source_ar }} @break
                @case('crm_stage') {{ $s->crm_stage_ar }} @break
                @case('organization') {{ $s->crmInfo->organization ?? '-' }} @break
                @case('exam_score') {{ $s->profile->exam_score ?? '-' }} @break
                @case('language_level') {{ $s->diplomas->pluck('pivot.language_level')->filter()->implode('، ') ?: '-' }} @break
                @case('certificate_agreement') {{ $s->diplomas->pluck('pivot.certificate_agreement')->filter()->implode('، ') ?: '-' }} @break
                @case('created_at') {{ $s->created_at?->format('Y-m-d') }} @break
                @default -
              @endswitch
            </td>
          @endforeach
        ...
        @empty
        <tr><td colspan="{{ count($availableColumns) }}" class="text-center text-muted py-5">لا يوجد نتائج</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const allCols     = @json(array_keys($availableColumns));
  ...
  document.getElementById('exportExcelBtn')?.addEventListener('click', function (e) {
    ...
    window.location.href = '{{ route('students.reports.exportExcel') }}?' + params.toString();
  });
