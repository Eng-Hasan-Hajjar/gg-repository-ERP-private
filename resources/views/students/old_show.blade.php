...
إغلاق
        ...
        <div class="row g-3 mt-1">
          <div class="col-md-6">
            ...
            <input type="number" step="0.01" min="0.01"
                   ...
                   placeholder="0.00">
            <small class="text-danger d-none installment-error-${i}">
              القيمة غير صحيحة
            </small>
          </div>
          <div class="col-md-6">
    ...
    // ── بعد بناء الحقول: أضف مراقب التحقق ──
    attachInstallmentValidation();
});

function attachInstallmentValidation() {
    ...
    function validate() {
        ...
        amountInputs.forEach((inp, idx) => {
            const val     = parseFloat(inp.value) || 0;
            const errEl   = document.querySelector(`.installment-error-${idx + 1}`);
            let fieldValid = true;
        ...
        // مجموع الدفعات لا يجوز أن يتجاوز الإجمالي
        const summaryEl = document.getElementById('installments-summary');
        if (summaryEl) {
            const remaining = total - sum;
            const overLimit = sum > total;

            summaryEl.innerHTML = `
              <div class="mt-3 p-3 rounded border ${overLimit ? 'border-danger bg-danger bg-opacity-10' : 'border-success bg-success bg-opacity-10'}">
                <div class="row text-center g-2">
                  ...
                  <div class="col-4">
                    ...
                    <div class="fw-bold ${overLimit ? 'text-danger' : ''}">${sum.toFixed(2)}</div>
                  </div>
                  <div class="col-4">
                ...
                ${overLimit ? '<div class="text-danger text-center small mt-2 fw-bold">⚠️ مجموع الدفعات يتجاوز الإجمالي!</div>' : ''}
              </div>`;

            if (overLimit) valid = false;
        }
