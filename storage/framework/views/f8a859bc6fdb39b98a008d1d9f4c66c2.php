...
<style>
  ...
  .stat-icon.online  { background: #d1fae5; color: #059669; }
  .stat-icon.offline { background: #fee2e2; color: #dc2626; }
  .stat-icon.hours   { background: #fef3c7; color: #d97706; }
  .stat-number {
    font-size: 28px;
    font-weight: 800;
    line-height: 1;
  ...
  /* ───── Table ───── */
  .roles-cell { min-width: 120px; }
  .role-line { display: block; margin-bottom: 4px; }
  .worked-cell { line-height: 1.3; white-space: pre-line; }
  .worked-hours { font-weight: 700; }
  .worked-minutes { font-size: 12px; color: #64748b; }
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 gap-2">
  ...
  <div class="d-flex gap-2 flex-wrap">
    <?php if(auth()->user()?->hasPermission('create_users')): ?>
      <a class="btn btn-primary rounded-pill px-4 fw-bold" href="<?php echo e(route('admin.users.create')); ?>">
        <i class="bi bi-person-plus"></i> مستخدم جديد
      </a>
    <?php endif; ?>
    <a href="<?php echo e(route('reports.monthly')); ?>" class="btn btn-success rounded-pill px-4 fw-bold">
...
<div class="stats-grid">
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-number"><?php echo e($onlineCount); ?></div>
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-number"><?php echo e($offlineCount); ?></div>
  ...
  <form method="GET">
    <div class="row g-2 align-items-end">
      <div class="col-12 col-md-3">
        ...
        <input name="search" value="<?php echo e(request('search')); ?>" class="form-control" placeholder="بحث بالاسم أو البريد">
      </div>
      ...
      <div class="col-6 col-md-2">
        ...
        <select name="online" class="form-select">
          ...
          <option value="1" <?= request('online') == '1' ? 'selected' : '' ?>>
            🟢 متصلون الآن (<?php echo e($onlineCount); ?>)
          </option>
        </select>
      </div>
      <div class="col-6 col-md-2">
      ...
      <div class="col-12 col-md-2 d-grid">
        <label class="form-label small mb-1">&nbsp;</label>
        <a href="<?php echo e(route('admin.users.index')); ?>" class="btn btn-outline-secondary fw-bold">
            ...
            <td class="text-end">
              <div class="action-btn-group">
                ...
                <button
                  ...
                  data-user-lastseen="<?= e($u->last_seen ?? '—') ?>"
                  ...
                  data-user-location="<?= e($locationText) ?>"
                  data-user-address="<?= e($fullAddress) ?>"
                  data-user-city="<?= e($sessionCity) ?>"
                  data-user-country="<?= e($sessionCountry) ?>"
                  data-user-ip="<?= e($sessionIp) ?>"
                  data-user-agent="<?= e($sessionAgent) ?>"
                  data-user-lat="<?= e($sessionLat) ?>"
                  data-user-lng="<?= e($sessionLng) ?>"
                  data-user-login="<?= e($loginTime) ?>"
                  data-user-created="<?= e($u->created_at?->format('Y-m-d')) ?>"
                >
                  <i class="bi bi-eye"></i> تفاصيل
                </button>

                <?php if(auth()->user()?->hasPermission('edit_users')): ?>
                  <a class="btn btn-sm btn-outline-primary" href="<?php echo e(route('admin.users.edit', $u)); ?>">
                    <i class="bi bi-pencil"></i>
                  </a>
                <?php endif; ?>

                <?php if(auth()->user()?->hasPermission('manage_roles')): ?>
                  <a class="btn btn-sm btn-outline-dark" href="<?php echo e(route('admin.roles.index', ['user' => $u->id])); ?>">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  modal.addEventListener('show.bs.modal', function(event) {
    ...
    <?php if(auth()->user()?->hasPermission('edit_users')): ?>
      actionsEl.innerHTML += '<a class="btn btn-sm btn-outline-primary rounded-pill px-3" href="<?php echo e(url("admin/users")); ?>/' + d.userId + '/edit"><i class="bi bi-pencil"></i> تعديل</a>';
    <?php endif; ?>

    <?php if(auth()->user()?->hasPermission('manage_roles')): ?>
      actionsEl.innerHTML += '<a class="btn btn-sm btn-outline-dark rounded-pill px-3" href="<?php echo e(url("admin/roles")); ?>?user=' + d.userId + '"><i class="bi bi-shield-lock"></i> الصلاحيات</a>';
    <?php endif; ?>
  });
});
</script>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\engya\Desktop\namaa\laravel11-auth\resources\views/admin/users/index.blade.php ENDPATH**/ ?>