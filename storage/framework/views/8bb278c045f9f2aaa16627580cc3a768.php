...
<style>
  ...
  /* ───── Grant (منحة) box inside selected diploma card ───── */
  .sd-grant-box {
    flex-basis: 100%;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderSelectedDiplomas() {
      ...
      selectedDiplomas.forEach((selection, name) => {
        ...
        card.innerHTML = `
                ...
                <div class="sd-grant-box">
                    <label class="sd-grant-check">
                        <input type="checkbox" class="diploma-grant-check" data-name="${name}" ${selection.hasGrant ? 'checked' : ''}>
                        <i class="bi bi-gift-fill"></i> منحة؟
                    </label>
                    <textarea class="form-control sd-grant-details diploma-grant-details" data-name="${name}"
                        rows="2" placeholder="ما هي المنحة؟"
                        style="${selection.hasGrant ? '' : 'display:none'}">${selection.grantDetails || ''}</textarea>
                </div>
            `;
        ...
        // ══ أحداث المنحة ══
        const grantCheck = card.querySelector('.diploma-grant-check');
        const grantDetailsField = card.querySelector('.diploma-grant-details');

        grantCheck.addEventListener('change', function () {
          selection.hasGrant = this.checked;
          grantDetailsField.style.display = this.checked ? 'block' : 'none';
          if (!this.checked) {
            selection.grantDetails = '';
            grantDetailsField.value = '';
          }
          updateHiddenInputs();
        });

        grantDetailsField.addEventListener('input', function () {
          selection.grantDetails = this.value;
          updateHiddenInputs();
        });
      });
    }
    ...
    function updateHiddenInputs() {
      ...
      selectedDiplomas.forEach((selection, name) => {
        ...
        // ══ بيانات المنحة الخاصة بهذه الدبلومة ══
        const grantInput = document.createElement('input');
        grantInput.type = 'hidden';
        grantInput.name = `diploma_grants[${selection.variantId}][has_grant]`;
        grantInput.value = selection.hasGrant ? '1' : '0';
        hiddenInputs.appendChild(grantInput);

        const grantDetailsInput = document.createElement('input');
        grantDetailsInput.type = 'hidden';
        grantDetailsInput.name = `diploma_grants[${selection.variantId}][grant_details]`;
        grantDetailsInput.value = selection.grantDetails || '';
        hiddenInputs.appendChild(grantDetailsInput);
      });
    }
