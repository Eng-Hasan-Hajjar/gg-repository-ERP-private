...
class ReportsRepository
{
    ...
    {
        // ✅ Cache نتيجة Schema::hasTable - لا تستعلم قاعدة البيانات في كل مرة
        return Cache::remember("schema_has_{$table}", 3600, fn() => Schema::hasTable($table));
    }

    // ✅ موجودة في الأصل - محفوظة كما هي
    ...
    public function studentsTotal(?int $branchId = null): int
    {
        ...
        $key = "students_total_{$branchId}";
        return Cache::remember($key, 300, function() use ($branchId) {
    ...
    {
        if (!$this->has('students')) return 0;
        return Cache::remember("students_confirmed_{$branchId}", 300, function() use ($branchId) {
    ...
    {
        if (!$this->has('students')) return 0;
        return Cache::remember("students_unconfirmed_{$branchId}", 300, function() use ($branchId) {
            $q = DB::table('students')->when($branchId, fn($x) => $x->where('branch_id', $branchId));
            if (Schema::hasColumn('students', 'is_confirmed')) {
    ...
    public function employeesTotal(?int $branchId = null): int
    {
        ...
        if (!$table) return 0;
        return Cache::remember("employees_total_{$branchId}", 300, function() use ($table, $branchId) {
    ...
    public function employeesActive(?int $branchId = null): int
    {
        $table = $this->has('employees') ? 'employees' : ($this->has('staff') ? 'staff' : null);
        if (!$table) return 0;
        return Cache::remember("employees_active_{$branchId}", 300, function() use ($table, $branchId) {
    ...
    public function employeesInactive(?int $branchId = null): int
    {
        $table = $this->has('employees') ? 'employees' : ($this->has('staff') ? 'staff' : null);
        if (!$table) return 0;
        return Cache::remember("employees_inactive_{$branchId}", 300, function() use ($table, $branchId) {
            $q = DB::table($table)->when($branchId, fn($x) => $x->where('branch_id', $branchId));
            if (Schema::hasColumn($table, 'is_active')) {
                $q->where('is_active', 0);
            } elseif (Schema::hasColumn($table, 'status')) {
    ...
    public function employeesPresentToday(?int $branchId = null): int
    {
        ...
        $key = "present_today_{$branchId}_" . now()->toDateString();
        return Cache::remember($key, 120, function() use ($branchId) {
    ...
    {
        if (!$this->has('attendances')) return 0;
        $key = "absent_today_{$branchId}_" . now()->toDateString();
        return Cache::remember($key, 120, function() use ($branchId) {
            $q = DB::table('attendances');
            if (Schema::hasColumn('attendances', 'date')) {
                $q->whereDate('date', now()->toDateString());
            ...
            if (!Schema::hasColumn('attendances', 'status')) return 0;
    ...
    public function tasksDueToday(?int $branchId = null): int
    {
        ...
        $key = "tasks_due_today_{$branchId}_" . now()->toDateString();
        return Cache::remember($key, 120, function() use ($branchId) {
            $q = DB::table('tasks');
    ...
    public function boxesTotal(?int $branchId = null): int
    {
        ...
        if (!$table) return 0;
        return Cache::remember("boxes_total_{$branchId}", 600, function() use ($table, $branchId) {
            return (int) DB::table($table)
    ...
    public function paymentsSum(string $from, string $to, ?int $branchId = null): float
    {
        ...
        if (!$table) return 0;
        $key = "payments_sum_{$from}_{$to}_{$branchId}";
        return Cache::remember($key, 300, function() use ($table, $from, $to, $branchId) {
            ...
            if (!$dateCol) return 0;
            $q = DB::table($table)->whereDate($dateCol, '>=', $from)->whereDate($dateCol, '<=', $to);
    ...
    public function studentsPerBranch(): array
    {
        ...
        return Cache::remember('students_per_branch', 600, function() {
    ...
    public function revenueToday(?int $branchId = null): float
    {
        $table = $this->has('payments') ? 'payments'
            : ($this->has('transactions') ? 'transactions' : null);
        if (!$table) return 0;
        $key = "revenue_today_{$branchId}_" . now()->toDateString();
        return Cache::remember($key, 120, function() use ($table, $branchId) {
            $amountCol = Schema::hasColumn($table, 'amount') ? 'amount'
                : (Schema::hasColumn($table, 'total') ? 'total' : null);
            $dateCol = Schema::hasColumn($table, 'paid_at') ? 'paid_at'
                : (Schema::hasColumn($table, 'date') ? 'date' : 'created_at');
            $q = DB::table($table)->whereDate($dateCol, now()->toDateString());
            if ($branchId && Schema::hasColumn($table, 'branch_id')) {
                $q->where('branch_id', $branchId);
            }
            return (float) $q->sum($amountCol);
    ...
    {
        return Cache::remember("latest_audit_{$limit}", 60, function() use ($limit) {
    ...
    public function revenuePerBranch(string $from, string $to, ?int $branchId = null): array
    {
        ...
        $key = "revenue_per_branch_{$from}_{$to}_{$branchId}";
        return Cache::remember($key, 300, function() use ($from, $to, $branchId) {
            $query = DB::table('branches')
                ...
                ->select('branches.name as branch', DB::raw('COALESCE(SUM(cashbox_transactions.amount),0) as total'))
                ->groupBy('branches.name')
                ->orderBy('branches.name');
            if ($branchId) {
            ...
            return $query->get()
                ->map(fn($r) => ['branch' => $r->branch, 'total' => (float) $r->total])
                ->toArray();
        });
    }
    ...
    public function studentsGrowth(): array
    {
        ...
        return Cache::remember('students_growth', 3600, function() {
    ...
    {
        if (!$this->has('students')) return [];
        return Cache::remember("students_growth_by_month_{$months}", 3600, function() use ($months) {
            return DB::table('students')
                ->selectRaw("DATE_FORMAT(created_at, '%Y-%m') as month_key, DATE_FORMAT(created_at, '%m/%Y') as month_label, COUNT(*) as total")
                ->where('created_at', '>=', now()->subMonths($months))
                ->groupBy('month_key', 'month_label')
                ->orderBy('month_key')
                ->get()
                ->map(fn($r) => ['month' => $r->month_label, 'total' => (int) $r->total])
                ->toArray();
        });
    }

    // =========================
    // ✅ systemAlerts() - مع Cache
    // =========================
    public function systemAlerts(?int $limit = null): array
    {
        ...
        // ✅ Cache لمدة 60 ثانية فقط لأن الإشعارات تحتاج تحديث
        $cacheKey = "system_alerts_{$userId}";

        $alerts = Cache::remember($cacheKey, 60, function() use ($user) {
            return $this->buildAlerts($user);
        });
    ...
    /**
     * ✅ منفصلة عن systemAlerts() للـ Cache
     */
    private function buildAlerts($user): array
    {
        ...
        // =============================
        // Leads - استعلام واحد مدمج بدل استعلامين
        // =============================
        if ($this->has('leads')) {
            $leads = DB::table('leads')
                ->leftJoin('lead_followups', 'lead_followups.lead_id', '=', 'leads.id')
                ->select(
                    'leads.id', 'leads.full_name', 'leads.first_contact_date',
                ...
                ', [$cutoff24, $cutoff24]) // ✅ استعلام واحد يجلب الاثنين
            ...
            foreach ($leads as $lead) {
                ...
                $isUrgent  = $hoursDiff >= $followupHours;
                ...
                $alerts[] = [
                    ...
                    'icon'        => $isUrgent ? 'bi-exclamation-triangle-fill' : 'bi-clock-history',
                    'message'     => $isUrgent
                        ? "{$lead->full_name} — لم تتم المتابعة ({$timeLabel})"
        ...
        // =============================
        // Leaves - استعلام واحد مدمج
        // =============================
        if ($this->has('leaves')) {
            $pendingLeaves = DB::table('leaves')
                ->join('employees', 'employees.id', '=', 'leaves.employee_id')
                ->select('leaves.id', 'leaves.start_date', 'leaves.end_date', 'leaves.type', 'leaves.created_at', 'employees.full_name as employee_name')
        ...
        // =============================
        // Asset Requests - استعلام واحد مدمج
        // =============================
        if ($this->has('asset_requests')) {
            // ✅ استعلام واحد يجلب urgent + normal معاً
            $allPendingRequests = DB::table('asset_requests')
                ->join('users', 'users.id', '=', 'asset_requests.user_id')
                ->leftJoin('branches', 'branches.id', '=', 'asset_requests.branch_id')
                ->select(
                    'asset_requests.id', 'asset_requests.title',
                    'asset_requests.type', 'asset_requests.priority',
                ...
                ->where(function($q) {
                    $q->where('asset_requests.priority', 'urgent')
                      ->orWhere('asset_requests.created_at', '>=', now()->subHours(24));
                })
                ->orderByDesc('asset_requests.priority') // urgent أولاً
                ->orderByDesc('asset_requests.created_at')
                ->get();

            $totalPending = DB::table('asset_requests')->where('status', 'pending')->count();
            ...
            foreach ($allPendingRequests as $req) {
                ...
                $alerts[] = [
                    'type'        => $isUrgent ? 'danger' : 'warning',
                    'icon'        => $isUrgent ? 'bi-exclamation-circle-fill' : 'bi-box-seam',
        ...
        $alerts = array_filter($alerts, function ($alert) use ($user) {
            ...
            if (!$hasPermission) {
                foreach ($alert['permissions'] as $perm) {
                    if ($user->hasPermission($perm)) { $hasPermission = true; break; }
                }
            }
    ...
    // =========================
    // Other methods with Cache
    // =========================
    public function todayAlertsSummary(): array
    {
        $key = 'today_alerts_summary_' . now()->toDateString();
        return Cache::remember($key, 120, function() {
            $result = ['pending_leaves' => 0, 'today_tasks' => 0];
            if ($this->has('leaves')) {
                $result['pending_leaves'] = DB::table('leaves')->where('status', 'pending')->count();
            }
            if ($this->has('tasks')) {
    ...
    {
        $key = 'today_system_stats_' . now()->toDateString();
        return Cache::remember($key, 120, function() {
            $stats = ['financial_transactions' => 0, 'financial_amount' => 0, 'new_students' => 0, 'confirmed_students' => 0];
            if ($this->has('cashbox_transactions')) {
                $stats['financial_transactions'] = DB::table('cashbox_transactions')->whereDate('created_at', now())->count();
                $stats['financial_amount']       = DB::table('cashbox_transactions')->whereDate('created_at', now())->sum('amount');
            }
            if ($this->has('students')) {
                $stats['new_students'] = DB::table('students')->whereDate('created_at', now())->count();
                if (Schema::hasColumn('students', 'confirmed_at')) {
                    $stats['confirmed_students'] = DB::table('students')->whereDate('confirmed_at', now())->count();
                }
            }
            return $stats;
        });
    ...
    public function highPriorityTasks(): array
    {
        ...
        return Cache::remember('high_priority_tasks', 120, function() {
    ...
    {
        if (!$this->has('audit_logs')) return ['count' => 0, 'last' => null];
        $key = 'system_activity_today_' . now()->toDateString();
        return Cache::remember($key, 120, function() {
            return [
                'count' => DB::table('audit_logs')->whereDate('created_at', now())->count(),
                'last'  => DB::table('audit_logs')->latest()->value('created_at'),
            ];
        });
    }

    private function kpiWithGrowth($current, $previous): array
    {
        if ($previous == 0) return ['value' => $current, 'growth' => 0];
        return ['value' => $current, 'growth' => round((($current - $previous) / $previous) * 100, 2)];
    }
}